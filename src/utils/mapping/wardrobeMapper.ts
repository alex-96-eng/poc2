...
import type { DoorConfig, FrameConfig, PanelConfig, UnleashedLineItem, Wardrobe } from "@/types";
...
export function mapWardrobeToUnleashedItems(
    ...
    const groups = doorDetails.reduce((acc, d) => {
        const key = normalizePanelName(d.panels[0].name); // FIXME: This only does the first one?
